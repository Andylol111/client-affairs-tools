...
- Intake, Beta, and Production are three different waterfalls. Intake tests human topic PRs into develop and never builds an image or deploys. Beta verifies via dispatch on develop, then ships the image on feature. Production verifies via dispatch on feature, then ships the live box on main. Bot-opened PRs do not start `pull_request` workflows (those wait for approval and expire). Trusted feature/main history is merged straight into develop; no `sync/*` topic branches.
