...
Bot promotion PRs use explicit stage dispatch rather than a second approval-gated `pull_request` run. Dependabot retains its real Intake PR checks. Successful same-repository topic PRs and allowlisted Dependabot PRs merge without an opt-in label. A draft, hold label, requested changes, foreign fork, stale head, unexpected dependency file, or closed unmerged candidate stops automatic merging. Merges bind to the checked head and never use an admin bypass. When an existing promotion PR follows a newer source revision, the controller dispatches that exact revision through the next stage instead of leaving stale checks attached.
