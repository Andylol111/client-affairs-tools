...
Staged-delivery work on `feat/staged-delivery-and-studio` is local. Do not approve a Production deploy of it until: the `beta` GitHub environment exists and targets a different instance than `i-09a071e22270b027c`; `PROMOTION_APP_ID` / `PROMOTION_APP_PRIVATE_KEY` are set; deploy preflight (`check_deployment.py`) accepts the target (IMDSv2, encrypted volumes, no public security-group ingress); and the production approval is for that exact SHA. A docs-only or workflow-only promotion still must pass `required-checks` but must not restart the live box.
